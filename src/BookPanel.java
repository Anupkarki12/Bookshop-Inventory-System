        ...
        BPanel()
            ...
            public void actionPerformed(ActionEvent en) {
                if (en.getSource() == add) {
                    try {
                        if (photo != null) {
                            Connection con = DriverManager.getConnection("jdbc:mysql://" + connData[0], connData[1], connData[2]);
            
                            // Prepare SQL statement with PreparedStatement
                            String sql = "INSERT INTO books (book_name, book_author, book_price, book_quantity, data_editor, data_edit_date) " +
                                         "VALUES (?, ?, ?, ?, ?, CURRENT_DATE)";
            
                            PreparedStatement pstmt = con.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS);
                            pstmt.setString(1, name_data.getText()); // Set book_name
                            pstmt.setString(2, author_data.getText()); // Set book_author
                            pstmt.setFloat(3, Float.parseFloat(price_data.getText())); // Set book_price
                            pstmt.setInt(4, Integer.parseInt(quantity_data.getText())); // Set book_quantity
                            pstmt.setString(5, new Login().UserNAME); // Set data_editor
                            ...
                            if (affectedRows > 0) {
                                ...
                                if (generatedKeys.next()) {
                                    int generatedISBN = generatedKeys.getInt(1);
                                    System.out.println("Generated ISBN: " + generatedISBN);
            
                                    // Move the uploaded photo to the appropriate location
                                    Files.copy(photo.toPath(), Paths.get(picLocation + generatedISBN + ".png"), StandardCopyOption.REPLACE_EXISTING);
            
                                    // Clear input fields and reset photo
                                    isbn_data.setText("");
                                    name_data.setText("");
                                    author_data.setText("");
                                    price_data.setText("");
                                    ...
                                    // Schedule hiding the success message after 2.25 seconds
                                    Timer a = new Timer();
                                    TimerTask a_task = new TimerTask() {
                                        @Override
                                        public void run() {
                                            warning.setVisible(false);
                                        }
                            ...
                            // Display error message if no photo is uploaded
                            warning.setText("Error: Please upload Photo");
                            warning.setForeground(Color.RED);
                            warning.setVisible(true);
            
                            // Schedule hiding the error message after 2.25 seconds
                            Timer a = new Timer();
                            TimerTask a_task = new TimerTask() {
                                @Override
                                public void run() {
                        ...
                        if (ex instanceof NumberFormatException) {
                            ...
                            warning.setText("Error: Database Connection Error");
                        } else if (ex instanceof IOException) {
                            warning.setText("Error: Failed to copy photo");
                        } else {
                            warning.setText("Error: Unexpected Error");
                        }
                        warning.setForeground(Color.RED);
                        warning.setVisible(true);
            
                        // Schedule hiding the error message after 1.5 seconds
                        Timer a = new Timer();
                        TimerTask a_task = new TimerTask() {
                            @Override
                            public void run() {
                ...
                else if(en.getSource()==cancel)
                    {
                        ...
                        try {
                            int bookISBN = 0;
                            String bookNAME = name_data.getText(); // JTextField for book name input
                            String bookAUTHOR = author_data.getText(); // JTextField for author name input
                            float bookPRICE = 0;
                            int bookQUANTITY = 0;
                            ...
                            // Determine the search criterion
                            String query = "";
                            if (!bookNAME.isEmpty()) {
                                query = "SELECT * FROM `books` WHERE book_name='" + bookNAME + "'";
                            } else if (!bookAUTHOR.isEmpty()) {
                                query = "SELECT * FROM `books` WHERE book_author='" + bookAUTHOR + "'";
                            }
                            ...
                            while (rs.next()) {
                                ...
                                // Set the book image using the book's name or ISBN
